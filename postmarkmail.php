<?php
/*
Plugin Name: PostMarkApp Mail Replacement
Plugin URI: http://tmertz.com/projects/postmarkmailreplacement/
Description: A simple plugin that replaces the default wp_mail function with PostMarkApp (<a href="http://postmarkapp.com/">http://postmarkapp.com/</a>). This plugin is proudly sponsored by <a href="http://stupid-studio.com/">Stupid Studio</a>.
Version: 1.3.2
Author: Thomas Mertz
	...
	function wp_mail($to, $subject, $body) {
		...
		if ( strstr($to, ",") ) {
			...
			foreach ($recipients as $recipient) {
				...
				$message = postmark_json_encode($recipient, $subject, $body);
			
				$headers = array(
					'Accept: application/json',
					'Content-Type: application/json',
					'X-Postmark-Server-Token: ' . get_option( 'pm_wp_mail_key' )
				);
				...
				postmark_return_log($httpCode,$return);
				
			}
			
		} else {
		
			$message = postmark_json_encode($to, $subject, $body);
...
function pm_wp_mail_plugin_menu() {
	add_menu_page('PostMarkApp', 'PostMarkApp', 'publish_posts', 'postmark-menu', 'pm_wp_mail_management','',150);
	add_submenu_page('postmark-menu', 'Logs', 'Logs', 'publish_posts', 'postmark-logs', 'pm_wp_mail_logs');
}
...
function pm_wp_mail_management() {
	...
	echo "<div class=\"wrap\">\n";
	echo "<div id=\"icon-tools\" class=\"icon32\"></div><h2>PostMarkApp</h2>\n";
	
	if( $_GET["action"] == "test") {
		$to = $current_user->user_email;
		wp_mail($to, 'PostMarkApp Mail Replacement Test', 'This is a test. If you are receiving this email PostMarkApp Mail Replacement is working as intended.');
	...
	echo "<tr valign=\"top\">\n";
	echo "<th scope=\"row\"><label for=\"pm_wp_mail_address\">PostMark email address</label></th>\n";
	echo "<td>\n";
	echo "<input type=\"text\" name=\"pm_wp_mail_address\" value=\"" . get_option( 'pm_wp_mail_address' ) . "\" size=\"60\" /><br /><span class=\"description\">Usually something like 'no-reply@yourdomain.com'.<br />Should match the sender signature you registered with PostMarkApp.</span>\n";
	echo "</td>\n";
	echo "</tr>\n";
...
function pm_wp_mail_logs() {
	...
	echo "<div class=\"wrap\">\n";
	echo "<div id=\"icon-tools\" class=\"icon32\"></div><h2>PostMarkApp Logs</h2>\n";
	...
	echo "<p><form method=\"post\" action=\"/wp-admin/admin.php?page=postmark-logs&action=purge\"><input type=\"submit\" class=\"button-primary\" value=\"Purge Logs\" name=\"submit\"></form></p>";
